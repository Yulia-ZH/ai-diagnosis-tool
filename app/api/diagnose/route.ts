...
function extractUrl(text: string): string | null {
  const match = text.match(/https?:\/\/[^\s\u4e00-\u9fa5]+/);
  return match ? match[0] : null;
}

// 将抓取到的数据格式化为可读文本
function formatAccountData(data: Record<string, string>): string {
  const lines: string[] = [`【真实抓取数据 - ${data.platform}】`];
  if (data.name) lines.push(`账号名称：${data.name}`);
  if (data.fans) lines.push(`粉丝数：${data.fans}`);
  if (data.likes) lines.push(`获赞与收藏：${data.likes}`);
  if (data.following) lines.push(`关注数：${data.following}`);
  if (data.noteCount) lines.push(`笔记数：${data.noteCount}`);
  if (data.bio) lines.push(`账号简介：${data.bio}`);
  if (data.recentTitles) lines.push(`近期内容标题：${data.recentTitles}`);
  if (data.recentLikes) lines.push(`近期内容点赞数：${data.recentLikes}`);
  if (data.error) lines.push(`注意：部分数据抓取失败（${data.error}），以下为可获取数据`);
  return lines.join("\n");
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 自动抓取账号真实数据
    let realData = "";
    const allText = [accountInfo, ...messages.map((m: { content: string }) => m.content)].join(" ");
    const url = extractUrl(allText);

    if (url && (url.includes("xhslink.com") || url.includes("xiaohongshu.com") ||
                url.includes("douyin.com") || url.includes("bilibili.com") || url.includes("b23.tv"))) {
      try {
        const fetchRes = await fetch(`${req.nextUrl.origin}/api/fetch-account`, {
          method: "POST",
        ...
        if (fetchRes.ok) {
          ...
          if (data && !data.error) {
            realData = formatAccountData(data);
          }
        }
      } catch {
        // 抓取失败不影响主流程
    ...
    if (realData) {
      systemContent += `\n\n${realData}\n\n重要：以上是通过爬虫实时抓取的真实数据，请严格基于这些真实数据进行分析，不要编造或推测数据。`;
    } else if (accountInfo) {
      systemContent += `\n\n用户提供的账号信息：\n${accountInfo}\n\n注意：未能自动抓取到该账号的实时数据，请基于用户提供的信息进行分析，并在回复中说明数据来源于用户输入。`;
    }
